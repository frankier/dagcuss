...
def view_post(post_id=None, page_num=None):
    if post_id:
        post = graph.posts.get(post_id) # TODO: Bulbs should check correct model type returned, in fact Bulbs should return a node and not a vertex
...
def register():
    ...
    if form.validate_on_submit():
        flash("Now just check your email to confirm your account and you can login.")
        user = graph.users.create(username=form.username.data, password=form.password.data, email=form.email.data, active=0) # TODO: hash password
